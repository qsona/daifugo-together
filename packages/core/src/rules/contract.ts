 ...
 * - `afterPass`: after a voluntary pass is accepted and before field-clear
 *   evaluation. Turn skips generated by `skipTurns` do not invoke this hook.
...
export interface RuleHooks {
  ...
  afterPass(context: RuleContext, pass: DeepReadonly<RulePass>): Effect[];
...
export type RuleInput =
  ...
  | { kind: 'integer'; choiceId: string; value: number }
  | {
      kind: 'miniGameResult';
      choiceId: string;
      miniGameId: string;
...
export interface IntegerChoiceRequest {
  kind: 'integer';
  player: PlayerId;
  choiceId: string;
  min: number;
...
/** A voluntary pass accepted from the active player. */
 ...
 * Hook permissions:
 *
 * | Effect | afterPlay | afterPass | afterFieldClear | onGameStart | onGameEnd |
 * | --- | --- | --- | --- | --- | --- |
 * | clearField / clearSuitBinding | yes | no | no | no | no |
 * | requestChoice (contract v2) | yes | no | no | yes | no |
 * | skipTurns / reverseTurnOrder / forceRank / moveCards | yes | yes | yes | yes | no |
 * | setMemory | yes | yes | yes | yes | set scope only |
 * | announce | yes | yes | yes | yes | yes |
 *
