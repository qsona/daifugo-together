...
本番管理画面は、利用者向けWebと同じFly Appで `https://daifugo-together.fly.dev/admin` に公開する。読み取り専用で、運用概要、提案一覧、ユーザー一覧を表示する。
...
ユーザートークンとGoogle subjectは管理APIへ返さない。初版では更新・削除などの管理操作を提供しない。
...
6. DevToolsのNetworkで `/admin/api/*` の応答に `userToken`、`googleSub`、`google_sub` が含まれない。
7. ログアウト後、管理APIがHTTP 401を返す。
