...
- 作成日: 2026-07-24
- 状態: 承認済み(スタックは 2026-07-25 開発者承認、2026-07-27 の改訂反映済み。§7 の決定待ち事項を除き確定)
...
flowchart TB
    ...
    subgraph Server["ゲームサーバー(Node.js 単一プロセス / Fly.io 1台)"]
        ...
        Registry["ルールレジストリ<br/>DB の有効フラグ・優先度を読み<br/>セット開始時にルールチェーンを固定"]
        Rules["ルール群(packages/rules)<br/>通常の TS モジュール(静的 import)<br/>RuleModule/Effect 契約に準拠(§4.6 改訂)"]
        AI["対戦 AI(packages/ai)<br/>非 LLM ロジック・手番ごとに CPU 予算内で探索"]
        API["HTTP API<br/>提案受付(送信時に同期検査 E6)・図鑑・評価"]
    end

    subgraph Data["永続化(同一 VM)"]
        DB[("SQLite<br/>ユーザー/提案/ルールメタ(有効フラグ)/評価/結果")]
        ALog[("対局アクションログ(追記専用)<br/>ReplayInit + アクション列(§4.4)")]
    ...
    subgraph Pipeline["自動実装パイプライン(pipeline ワーカー)"]
        ...
        CI["GitHub PR + CI<br/>型検査/テスト/差分ガード/シミュレーション"]
        Deploy["マージ後: 通常デプロイで反映<br/>(graceful restart = ゲーム単位 §4.5)"]
    end
    ...
    API -->|"検査 pass のみ保存(E6)"| DB
    Engine -->|"結果・評価の書込"| DB
    Engine -->|"アクション追記"| ALog
    Registry -->|"有効フラグ・優先度"| DB
    DB --> Backup
    ALog --> Backup
    API --> Queue
    Queue --> Judge
    ...
    Codex --> CI
    CI --> Deploy
    Deploy -.->|"新バージョンの Server(ルール込み)"| Server
```
...
| 対戦 AI | 手番ごとに合法手を列挙し予算内で選択(**非 LLM のロジック探索**。方式は E2)。エンジン経由でルールチェーンを参照 | **LLM は使わない**(企画書 §3.3 の確定方針どおり、対局処理で LLM・外部 AI API を一切呼ばない)。ルールの独自解釈もしない(エンジンの判定に従う) |
...
| **進行中の対局状態** | **しない**(メモリのみ) | クラッシュで消える割り切り(§3)。復旧が必要になったら下のアクションログのリプレイで実現する拡張余地を残す |
| **対局アクションログ**(ReplayInit + アクション列。E1 のリプレイ形式) | **する**(追記専用) | 2026-07-25 開発者決定で要件に昇格。エンジンが決定的(E1)なので、このログだけで任意の対局をバイト単位で再現できる — チューニング・問題調査の一次資料。1 ゲーム数 KB・書き込みは毎秒数件以下。保存先(DB テーブル or JSONL ファイル + Litestream)と保持期間は E1/E10 で確定 |
...
**前提: 趣味プログラミングの規模で始める。** インフラは「専用の構築作業・OS 運用・IaC を持たない」「月額は缶ジュース数本程度」「やめたくなったら消すだけ」を満たすものから選ぶ。なお §4.3 で推奨した Node.js + Socket.IO は**「普通の常駐プロセス 1 個」であり、特別なインフラを何も要求しない** — 必要なのは「Node が常駐できて WebSocket が通る PaaS の 1 枠 + 永続ボリューム(SQLite ファイル置き場)」だけで、マネージド WebSocket サービス・複数台構成・Kubernetes の類いは一切不要である。比較は価格に加えて**初回セットアップと日常運用の手間**を明示する。

| 案 | 手軽さ(初回 / 日常) | 評価 |
|---|---|---|
| **A. Fly.io 1 台(推奨)** | 初回: `fly launch` + ボリューム作成の数コマンド(CLI 中心。Dockerfile は自動生成に任せてよい)。日常: `fly deploy` 1 コマンド(GitHub Actions で自動化) | 常駐 Node + WebSocket + 永続ボリューム(SQLite)が素直に動く。shared-cpu の小型マシンで月数ドル(256MB で $2 前後、実用構成で $5〜15/月程度)。東京リージョン(nrt)あり |
| **B. Railway(手軽さ最優先の代替)** / Render | 初回: **GitHub リポジトリを繋ぐだけ**(Dockerfile 不要・CLI 不要)で最も手軽。日常: git push で自動デプロイ | Railway Hobby $5/月 + 永続ボリュームで A と同等が動く。**「アカウントを作って繋ぐだけ」を優先するなら B(Railway)で始めてよい** — 東京リージョンの有無だけ契約時に要確認(ターン制ゲームなので近隣リージョンでも体感上は許容)。Render の無料 Web サービスはアイドルでスリープするため常駐部屋サーバーに不向き。 |
| C. Cloudflare Workers + Durable Objects | 初回から独自の実行モデル(DO)への適応が必要 | 無料枠が厚く(リクエスト 10 万/日など)WebSocket Hibernation もあり魅力的だが、**リクエストあたり CPU 時間制限が AI 探索(§6)と相性が悪く**、常駐プロセス前提の pipeline ワーカーも置けない。実行モデルが特殊で判断基準 1(codex の扱いやすさ)にも不利。見送り(将来、観戦配信など読み取り系のスケールが必要になったときの拡張余地として記録) |

素の VPS(Hetzner 等)は最安だが OS 運用が自分持ちになるため、判断基準 2 で A・B に譲る。

**A と B の使い分け**: 推奨は A(東京リージョン + ボリューム運用の実績)だが、差は僅かで、**B(Railway)で始めて必要になったら A へ移る経路を公式の代替とする**。移行コストは小さい — 運用物は「Node プロセス 1 個 + SQLite ファイル 1 個」で変わらないため、引っ越しはファイルのコピーと DNS 切り替えだけである。どちらを選んでもアプリケーションコードは 1 行も変わらない。

**デプロイ方式**:
- **フロントエンド**: ビルド済み静的ファイルをゲームサーバー自身が配信する(同一オリジン。CORS・ドメイン設定が消え、運用物が増えない)。CDN 分離は必要になってからでよい。
- **エンジン/サーバー本体・ルール**: GitHub Actions から `fly deploy`(main へのマージで自動)。ルールの反映もデプロイで行う(§4.6 改訂)。**graceful restart はゲーム単位**(2026-07-25 開発者決定): SIGTERM を受けたら draining モードに入り、新規ゲームの開始を止め、**進行中のゲーム(1 ゲーム ≒ 2 分)だけは完走させて**から exit する。`fly.toml` の `kill_timeout = 300`(shared-cpu の上限 5 分)を保険とし、ゲーム単位ならこの窓に収まる。**セットは途中打ち切りを許容** — 打ち切り時はそこまでの結果でセットリザルト+評価画面へ進む(詳細仕様は E1/E3 で確定。評価は「そのセットで発動したルール」対象なので途中終了でも成立する)。なお active_sets=0 を待つ deploy gate は**採らない**(流行するとデプロイ不能になるため)。
...
> **改訂(2026-07-25 開発者決定): ルールの反映方式を「デプロイあり・疎結合」に変更する。**
>
> - **ルール = リポジトリ内の通常 TS モジュール**(packages/rules/ 配下)。静的 import で本体に組み込み、**反映は通常のサーバーデプロイ**で行う(graceful restart は §4.5)。
> - **RuleModule / Effect 契約((2) 以降)はソフトウェア設計規約として全面維持**する(セキュリティ境界としてではなく)。E9 の優先順位・競合解決、E1 の決定性・リプレイ、対戦 AI の自動追従はこの契約の上に定義されているため。契約で表現できないルール(7渡し・10捨て等の入力要求系)は「契約自体を拡張するエンジン変更」として扱う(choice 機構が第 1 号候補。E1 §2.11)。
> - **ルール単位の有効/無効 DB フラグは維持**する。コードはデプロイで入るが発動は DB フラグで制御 — 壊れたルールの即時無効化(CX-04)と低評価排除(EV-03)は再デプロイなしで効き続ける。
> - **QuickJS サンドボックス(§4.8)と動的バンドル配布は Stage 1(人手ゼロの全自動マージ)検討時まで棚上げ**。それまでの安全は Stage 0 の人手レビュー + CI(差分ガード・型・テスト・シミュレーション)が担う。
>
> 理由: (1) 「わけのわからないルール」に賭ける以上、契約(Effect 語彙・フック)は今後何度も壊して育てることになる。デプロイ方式なら契約変更とルールが常に同じコミットで揃い、登録済みバンドルとの版ズレ問題が消える。(2) QuickJS・バンドルのビルド/登録/動的ロード機構が丸ごと不要になり、初期実装が大幅に軽い。ルールは「普通の TS モジュール」になり codex にも最も書きやすい。(3) Stage 0(人手レビュー)を維持する限り、サンドボックスは趣味規模には過剰な防御である。
>
> トレードオフ(正直に): 実行時保護は弱まる — ルール内の例外は try/catch + DB フラグ無効化で受けられるが、**無限ループ・メモリ暴走はプロセス内では止められない**。防衛線は CI のシミュレーション・エンジン側の手数上限ガード(E1)・Stage 0 レビューになる。また反映のたびにデプロイが要る(§4.5 の graceful restart 設計とセット)。
>
> 波及: E1(契約は維持・ロード機構が簡素化。アクションログ永続化 §4.4、デプロイ時のセット打ち切り仕様)、E2(サンドボックス境界コストが消え探索予算が大幅に緩む。§6 注記)、E7(バンドル登録 → 通常デプロイに置換。差分ガード・CI・Stage 0→1 の骨格は不変)、E3/E8(セット打ち切り時の遷移・評価)、E6(D4 の読み替え)。**各設計書の冒頭に改訂反映ノートを追記済み(2026-07-25)** — 本文の詳細な書き直しは実装着手時に該当節単位で行う。

以下の比較表と (1)〜(4) は初版の検討記録として残す(サンドボックス方式は Stage 1 検討時の再評価対象)。
...
export interface RuleHooks {
  ...
  /** 任意パス受理直後。skipTurns による内部スキップでは呼ばない */
...
**choice 機構の決定(2026-07-31、2026-08-31 拡張)**: 案 A を採用し、contract v2 の
`requestChoice → awaitingChoice → ruleInput → 同じ afterPlay / onGameStart の再呼び出し`
を実装した。本人の手札からのカード選択、列挙候補からのプレイヤー選択、
範囲付き整数選択、共通ミニゲームを扱う。範囲付き整数は安全な整数で、最大100候補、
範囲内の既定値を持ち、AI・切断代行・タイムアウトは既定値を選ぶ。任意文字列などの
自由入力・宣言、および `afterPass` からの入力要求は対象外。同じプレイで異なる
ルールが要求した場合は優先順位順に直列処理し、先行Effect適用後の状態から後続要求を
再計算する。詳細は
...
> **本節は棚上げ(2026-07-25 開発者決定)**: §4.6 改訂により、サンドボックス実行は Stage 1(全自動マージ)を検討する時点での再評価対象とする。以下は選定当時の比較・検討の記録として残す。
...
| DB | SQLite(better-sqlite3)+ Drizzle ORM + Litestream バックアップ。対局状態は永続化しないが、**対局アクションログは追記永続化**(リプレイ再現用。§4.4) |
| ホスティング | Fly.io 1 台(東京 nrt、SQLite ボリューム同居)。SPA は同一サーバーから配信。**手軽さ最優先なら Railway 開始 → 必要時に移行、を公式代替とする**(§4.5。アプリコードは不変) |
| ルールプラグイン | 1 ルール = 1 ディレクトリの通常 TS モジュール(静的 import)。RuleModule 契約 + Effect 語彙は設計規約として維持。**反映はデプロイ**(ゲーム単位 graceful restart)、有効/無効・即時無効化は DB フラグ(§4.6 改訂) |
| サンドボックス | **棚上げ**(Stage 1 検討時に再評価。§4.6 改訂)。それまでは Stage 0 人手レビュー + CI が防衛線 |
| codex パイプライン | 常駐ワーカーが `codex exec` を隔離ワークスペースで起動 → PR → CI(差分ガード/型/テスト/シミュレーション)→ 段階的 auto-merge → **デプロイで反映**(graceful restart は §4.5) |
| 認証 | 匿名 ID + トークン(提案時の強化はフェーズ 2 頭で判断) |
| CI/CD | GitHub Actions(テスト・差分ガード・バンドル発行・`fly deploy`) |

このスタックの一貫した性格: **「動くものは 1 台の Node プロセスに集約し、ルールは契約に従う小さなモジュールとして疎結合に増やす(発動の on/off はデータ = DB フラグ)」**。運用物を最小に保ちながら(判断基準 2)、毎日増えるルールをデプロイなしで安全に抜き差しできる(判断基準 3)。全部が TypeScript の 1 リポジトリなので、codex には「契約に従うディレクトリを 1 つ足す」仕事だけが渡る(判断基準 1)。

## 6. 性能・コストの見積もりの目安

> **注記(2026-07-25)**: §4.6 改訂によりルールはネイティブ実行になったため、本節の「サンドボックス境界コスト」を含む探索予算の見積もりは**保守側に倒れた旧前提**である(実際はこれより大幅に速い)。TS-03 の計測条件はネイティブ実行前提に更新すること。E2 の性能設計にも同様の緩和が波及する。
...
> **一元化のお知らせ(2026-07-25)**: 本節を含む全設計書の未決事項・Epic 間連絡は **[docs/decision-log.md](../decision-log.md)** に集約した。着手前の確認は台帳側で行うこと(台帳と本節が食い違う場合は設計書を正とし、台帳を直す)。
> **決定済み**: スタック承認(A-1)、リポジトリは **public**(A-2)。
...
7. **プレイヤー追加入力は案 A で決定済み(2026-07-31、2026-08-31 拡張)**。contract v2 でカード・プレイヤー・範囲付き整数・共通ミニゲームの choice を採用済み。任意文字列などの自由入力・宣言は今後の拡張候補とする(§4.6(2))。
8. **ルール実行時エラー・上限超過時の自動無効化の閾値**(§4.8。実装時に決定)。

## 8. バックログ追加案

E12 に紐づけるユーザーストーリー案。**`docs/product-backlog.md` へ転記済み(2026-07-25。TS-03 は §4.6 改訂に合わせネイティブ実行+worker_threads 前提に書き直して転記。バックログ側を正とする)。**転記時はストーリー本体に加えて、**バックログ §2 の Epic 一覧表に E12 の行(名称・狙い・フェーズ・企画書の節)を追加する**こと(これを忘れると Epic 一覧とストーリーの対応が崩れる)。
