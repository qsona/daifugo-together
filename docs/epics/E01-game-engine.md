...
>
> **MP-03 改訂ノート(2026-08-01)** — ゲーム間リザルトの既定待ち時間は **15 秒**を維持する。人間が 1 人ならその人の押下時、人間が複数なら全員の押下時に早く進める。押下が揃わなくても server タイマーの期限で必ず次戦を開始し、server が確定した終了時刻を snapshot に含めて表示バーと遷移時刻を一致させる。
...
- `interimResult` から次戦への遷移は `{ type: "advance" }` アクションで行う。**タイマーと人間の準備完了集約は I/O なのでエンジンは持たない**。server は人間が 1 人ならその人の準備完了時、人間が複数なら全員の準備完了時、または `interimAutoAdvanceMs`(既定 15 秒)の期限到達時に `advance` を投入する。server は確定した終了時刻も snapshot に含め、画面 5a の残量バーと遷移を同じ時計に従わせる。
...
### 2.11 choice 機構(プレイヤー追加入力)

2026-07-31 に E12 の案 A を採用し、contract v2 としてカード選択を実装した。
正準の状態遷移、秘匿性、AI・タイムアウト、互換性の仕様は
`docs/specs/2026-07-31-rule-choice-contract-v2-design.md` とする。

- `afterPlay` の `requestChoice` は、対象プレイヤー自身の残り手札から正確な枚数を要求する。
...
- v2 の1ルールあたりの範囲はカード選択1件だけであり、自由入力・宣言・入れ子の選択・パス起点入力は対象外とする。異なるルールの独立した1件ずつの選択は直列処理できる。
...
3. 第 i 戦(i < N−1)終了 → `interimResult` フェーズ。スナップショットの `setResults` に第 i 戦までの順位が入り、画面 5a が表示できる。server は全人間の準備完了時、または 15 秒後(`interimAutoAdvanceMs`)に `advance` を投入し、第 i+1 戦を開始する(BR-11: 先手は前戦の大貧民)。
...
- 画面 5a(ゲーム間リザルト): アプリバー「第 n 戦 おわり / セット i / N 戦」= `gameIndex` と `config.gamesPerSet`。順位一覧は `setResults[last].standings`(称号タグ・AI バッジ付き)。人間が 1 人なら「第 n+1 戦へ」の押下時、人間が複数なら全員の押下時に次戦を開始する。押下済みの人には「ほかのプレイヤーを待っています」と無効状態のボタンを表示する。押下が揃わなくても全員共通の 15 秒タイマーで次戦を開始する。評価入力は置かない(企画書 §4.4)。
...
契約で直接表現できない提案の扱いを、CX-01 の線引き資料に明記する:

| 却下区分 | 該当する提案 | 根拠 |
|---|---|---|
| contract v2 で対応 | 10捨てなど「自分の残り手札から正確な枚数を選ぶ」系 | §2.11。approve 対象 |
| choice 語彙外 | 自由入力・宣言・相手選択・複数段選択 | §2.11。reject ではなく needs_review |
