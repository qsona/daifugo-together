...
- v2 の範囲は、各対象者が本人の手札から選ぶ固定済みのカード選択である。1回の発動で複数対象者への要求を宣言でき、同一ルール内は宣言順、異なるルール間は優先順位順に直列処理する。自由入力・宣言・選択結果で次の要求が変わる入れ子の選択・パス起点入力は対象外とする。
...
| choice 語彙外 | 自由入力・宣言・対象者を選ぶ入力・動的な複数段選択 | §2.11。固定済みの複数対象者が各自の手札から選ぶ要求は対応済み。語彙外は reject ではなく needs_review |
