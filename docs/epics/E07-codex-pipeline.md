...
- 資料部: (1) Effect 語彙とフック一覧の要約 + **契約の表現限界**(contract v2 の `requestChoice` は対象者本人の残り手札から正確な枚数を選ぶ入力を扱う。固定済みの複数対象者や異なるルールの独立した選択は直列化できるが、選択結果で次の要求が変わる複数段選択・自由入力・宣言、状態形状や未対応手型の追加は語彙拡張候補) (2) 下記の線引き基準表 (3) 既存ルール一覧(name + summary。重複判定用。上限 100 件、超えたら要約リストを E11 のデータから生成)。
...
| 区分 | 細分類 | 判定基準 | 例 | 判定 |
|---|---|---|---|---|
| 契約拡張候補 | A1 追加入力要求 | 対象者本人の残り手札から正確な枚数を選ぶ入力は contract v2 で実装可能。固定済みの複数対象者は宣言順、異なるルールは優先順位順に直列化する。それ以外の自由入力・宣言・対象者を選ぶ入力・動的な複数段選択は追加語彙が必要 | 10捨てと全員が各自の手札を捨てるQボンバーは approve。渡す相手も選ぶ7渡しは needs_review | approve または needs_review |
