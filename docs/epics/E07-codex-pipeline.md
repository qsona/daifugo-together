...
> **マージ後工程の自動化(2026-08-11)** — マージ後に残っていた人手操作
> (マージの記録・`scripts/release.sh` によるデプロイ・有効化の個別承認)を廃止し、
> **人間のゲートを「PR レビュー → マージ」の 1 回に集約**した。開発者が GitHub 上で
> マージした行為が、マージ記録・デプロイ・ルール有効化までの一括承認になる。
> 実装 skill は `implement:await-merge`(`gh pr view` のポーリング)でマージを検知し、
> 以降は確認なしで自動連鎖する: マージ記録 → `implement:deploy` が `origin/main` を
> release ブランチへ fast-forward push → release の CI 成功 → CD が本番デプロイ →
> 起動時同期で登録 → `implement:release` が provenance 一致を検知して自動有効化
> (§2.2 / §2.6)。48 時間放置リマインダーは廃止し、セッション中断からの回復は
> 同じチェーンの再実行で行う(全段冪等)。**変更していないもの**: provenance 不一致時に
> 有効化を強行しない原則、CI 失敗・実装失敗時の開発者判断、skill が自分でマージしない
> (Never merge automatically)、§2.7 の人間レビューチェックリスト。
...
| E1(エンジン) | 依存 | RuleModule 契約・Effect 語彙・フック発火タイミング表(`packages/core/src/rules/README.md`)、シミュレーションハーネス、レジストリのセット開始時チェーン構築。E7 は「契約の消費者」であり契約自体は E1 が確定する |
...
| リリース | `released` | ルールが**有効化**された時点(マージ後の自動連鎖で実装 skill が有効化する。§2.6) |
...
sequenceDiagram
    ...
    alt カード / 却下
        ...
        D->>G: 開発者レビュー(§2.7)→ マージ(人間のゲートはここだけ)
        G-->>D: implement:await-merge がマージを検知(ポーリング)
        D->>S: マージを記録(phase=merged)
        D->>G: implement:deploy: origin/main を release へ fast-forward push
        G->>G: release の CI 成功
        G->>S: CD: 通常デプロイ(graceful restart・E13)
        S->>S: 起動時同期: 新ルールを rules に登録(有効化待ち)
        D->>S: implement:release が provenance 一致を検知 → 自動有効化 → proposals.released + RP-03 通知
...
| 5 | レビュー・マージ | 開発者 | green PR + チェックリスト(§2.7) | main 反映(**以降のマージ記録・デプロイ・有効化の一括承認**。§2.7) | 指摘 → 修正再実行 or 打ち切り。コンフリクトは構造上ほぼ発生しない(新規ディレクトリのみ) |
| 6 | デプロイ・登録 | 実装 skill(`implement:deploy`)→ CD(E13)→ server 起動時同期 | 実装 skill の `implement:deploy` による release ブランチへの fast-forward push(出荷単位はその時点の `origin/main` 全体) | `rules`/`rule_versions` 行(有効化待ち) | デプロイ失敗は E13 の CD 運用に従う。同期は冪等。再実行は `already_deployed` を返して次へ進む(§2.6) |
| 7 | 有効化・反映(CX-05) | 実装 skill(マージを承認とみなす)+ server | ジョブに記録された mergeSha と登録済み provenance の一致 → `rules.status=active` | 次セットからルール適用、`released` | provenance 不一致なら有効化せず開発者へ提示。ルールのロード・初期化失敗はそのルールだけスキップ + incident 記録 + 開発者通知 |
...
| **実装 skill** | 開発者マシンの Codex App 通常セッション | CX-02〜03 の駆動(prepare → セッション自身による実装 → submit → PR → CI 監視)と、レビュー材料の提示、マージ検知(`implement:await-merge`)、デプロイ(`implement:deploy`)、自動有効化(`implement:release`) | 開発者が手動起動 |
...
- **ツールとサーバーの通信**: サーバーが admin API(HTTPS + Bearer トークン)を提供し、両ドライバはこれ経由で読み書きする(SQLite の単一ライタをサーバープロセスに限定する方針は維持)。主なエンドポイント: `GET /admin/pipeline/screening`(未判定提案の払い出し。sanitized 本文 + L0〜L2 シグナル)/ `POST /admin/proposals/{id}/check`(L3 + 決定表の結果記録)/ `POST /admin/proposals/{id}/judge`(判定記録と、カード・却下・approve の開発者確定)/ `POST /admin/proposals/{id}/approve-spec`(SPEC 承認 → queued)/ `POST /admin/proposals/{id}/amend-spec`(レビュー前の承認 SPEC 改訂。元の承認へ紐づく developer judgement を追記し、`queued` / `implementing` / `pr_open` だけ許可。queued の払い出しは最新 judgement を読むため、改訂前 SPEC は実装へ渡らない)/ `GET /admin/pipeline/next`(次の queued ジョブの払い出し)/ `POST /admin/pipeline/jobs/{id}/update` / `POST /admin/pipeline/jobs/{id}/fail` / `POST /admin/rules/{id}/enable | disable`。
...
# 実装ガイド
packages/core/src/rules/README.md を必ず読んでから実装する。次が含まれる:
...
- **契約ドキュメント(`packages/core/src/rules/README.md`)は E1 の成果物**。プロンプトには全文を埋め込まず、ワークスペース内のファイルとして読ませる(リポジトリと常に同期し、二重管理を避ける)。「Effect 語彙にない作用は実装できない」等の要点はプロンプトにも直書きする(読み飛ばしへの保険)。
- E7 から E1 への要求事項: `packages/core/src/rules/README.md` に (a) フックごとの発火タイミングと典型ルール対応表、(b) Effect 語彙の網羅リスト、(c) 実装例 2〜3 件、(d) テスト用フィクスチャ(`RuleContext` ビルダー)の使い方、を含めること。またドキュメント外の要求として、(e) **本番エンジン自身に手数上限・強制終了規定を持たせる**こと(CI シミュレーションの「必ず終了する」不変条件と同じ上限値を共有する。E01 §5.1-3 / B-2 で対応済みの前提を確認)。これは §3.1 の「B1(進行破壊)の疑いは approve 側に倒す」非対称原則の前提であり、サンドボックスを持たないネイティブ実行(E12 §4.6 改訂)では、CI の検出漏れが本番の無限対局にならないための**最終防衛**である。
...
| simulation | E1 のハーネスで自動対局。大量不変条件検証は固定 seed のランダム合法手で各 200 ゲーム × 5 seed、AI 互換性 smoke は本番と同じ先読み長の対戦 AI で各 20 ゲーム × 1 seed。構成: (a) 基本ルール + 新ルールのみ (b) リポジトリ内の全ルール(`packages/rules/rules-exclude.json` 記載分を除く)+ 新ルール。不変条件: ゲームが上限手数内に必ず終了する / カードが増殖・消失しない / 不正な状態遷移がない / ルールの例外・無効 Effect が 1 件もない。AI smoke は合法手・safe/fast 一致・異常 fallback ゼロも検査する。**ジョブ自体にタイムアウトを設定する** — ネイティブ実行では無限ループを内側から止められないため、ジョブタイムアウトが検出器を兼ねる | 進行破壊・共存破壊、および AI-02 互換性 |
...
- **運用方針(2026-08-29実装、decision-log C-14)**: release push の CI では、前回 release から変更された各ルールの `new-only` と全ルール同時有効の `all-rules` を、ランダム合法手の不変条件検証 200 ゲーム × 5 seed と、AI 互換性 smoke 20 ゲーム × 1 seed に分けて検証し、両方成功するまでデプロイしない。ルール PR は `new-only` の両モードを 20 ゲーム × 1 seed で実行する。diff-guard・quality・rule-testsは維持する。
...
- TS-02 レビューの入口条件(decision-log G-4)を本 Epic 着手時に消化する: ①差分ガードの「単一新規ディレクトリ + 許可ファイルリスト」厳格化 ②PR 作成者検査(上記 (7)) ③**ガードのトリガー見直し**(現行 `pull_request` は PR 側がワークフロー定義自体を書き換え可能。`pull_request_target` 化 or required workflow 化)。
...
反映は**通常デプロイ**で行う(E12 §4.5/§4.6 改訂。graceful restart はゲーム単位、進行中セットは途中打ち切り許容)。**マージ以降は開発者の確認を挟まず自動で連鎖する**(2026-08-11 決定。マージが一括承認であり、人間のゲートは §2.7 のレビュー → マージの 1 回だけ):

1. **マージ検知 → デプロイ**: 実装 skill が `implement:await-merge`(`gh pr view` のポーリング。`merged` / `closed` / `changes_requested` / `pending` を返す)でマージを検知し、マージを記録する(`changes_requested` は既存の `pr_open` 修正ループへ戻り、`closed` は開発者へ提示する)。続けて `implement:deploy` が、`origin/main` が当該ジョブの mergeSha を含むことを検証したうえで、`origin/main` を **release ブランチへ fast-forward push** する(ローカル checkout の状態に依存しない)。release ブランチの CI が成功すると、それを待っている DP-02 の CD が本番へデプロイする。従来 `scripts/release.sh` を手で叩いていた操作の置き換えであり、**出荷単位はその時点の `origin/main` 全体**(当該ルールだけを切り出して出すわけではない)という意味論も従来と同じ。デプロイ時間帯への配慮(E-15: 遊ばれている時間帯を避ける)は、開発者がマージのタイミングを選ぶ判断に含まれる。
...
3. **有効化**: デプロイ後、`implement:release` が、起動時同期で登録された行の provenance(PR 番号・merge SHA など)がジョブの記録と一致することを確認し、一致していれば `POST /admin/rules/{id}/enable` を呼んで `rules.status='active'` にする。このとき `proposals.status=released` + RP-03 通知イベントを発行する。開発者の追加承認は求めない(マージが承認である)。**provenance が一致しない場合は有効化を強行せず**、開発者へ提示して判断を仰ぐ。
4. 反映(ルールチェーンへの組み込み)はセット開始時に行う(§3.5)。有効化に再デプロイは不要(DB フラグのみ)。
5. **中断からの回復**: セッションが途中で切れた場合は、同じチェーン(`implement:await-merge` → `implement:deploy` → `implement:release`)を再実行すればよい。全段が冪等で、済んでいる段は `already_recorded` / `already_deployed` / `already_released` を返して次へ進む。旧運用にあった「マージから 48 時間放置で開発者へリマインド」は skill の手順としてはこの再実行で代替できるため**廃止**した(サーバーが次ジョブ払い出し時に返す取り残しジョブ警告は、自動連鎖が止まったままであることの検知網としてそのまま残る)。
...
このチェックリストは恒久的な運用手順である(旧版の「Stage 0 限定・Stage 1 移行で廃止」という位置づけを廃し、**人間レビューをパイプラインの正式な一段**とする)。**マージは、マージ記録・デプロイ・ルール有効化までの一括承認である**(2026-08-11。マージ後は開発者の確認を挟まず自動で連鎖する — §2.6)。ここが人間のゲートの唯一の 1 回であることを前提に、上のチェックを済ませてからマージする。
...
**(a) 原文** —「ルール提案者として、採用された自分のルールが実際の対局で発動するところを見たい。それは『自分の提案でゲームが変わった』という手応えがこのゲームのコア体験だからだ。」受け入れ条件(2026-07-27 改訂、decision-log 参照): 反映済みルールが対局中に実際に発動する(反映先は全卓共通)/ 提案者は自分のルールが実装済み・有効であることを確認できる / **提案から反映までの開発者の操作が「実装 skill の起動・PR レビューとマージ」の 2 点に収まった実績が 1 件以上ある**(旧「人手の介在なしで完了」を置き換え。全自動化の凍結に伴う変更 — §1.1。2026-08-11 改訂: 有効化はマージ後の自動連鎖へ移行したため 3 点から 2 点に減った — §2.6)。
...
- 「開発者の操作 2 点以内」実績 1 件: リハーサル(§4-3)の正常系 1 件で、開発者の操作が skill 起動・レビューとマージの 2 点(+その中での対話)に収まったことを記録する(2026-08-11 改訂: 有効化はマージ後の自動連鎖へ移行 — §2.6)。
...
2. **結合テスト(外部をすべて fake に)**
   ...
   - 提案投入 → released / rejected / failed まで端から端を流し、DB の最終状態と通知イベントを検証する。人間の確定操作(却下確定・マージ)はテストドライバが admin API を叩いて代行し、マージ後の連鎖(マージ記録・デプロイ・有効化)は skill 側の実装をそのまま走らせる。CI 相当の検査はローカルで同じスクリプトを直接実行して代用。
   - **冪等性・回復**: 各段階の途中で skill プロセスを kill → 再起動 → 二重 PR・二重ブランチ・孤児ワークスペースが生じないこと。
3. **リハーサル(実環境での受け入れ試験)**
   - 実リポジトリ・実 codex・実 CI で、種類の異なるテスト提案を流す: 正常系ローカルルール(8切り相当)/ 正常系オリジナル / 契約外(A1: 7渡し)/ ゲーム破壊(B2: 手札全公開)/ 命令文を紛れ込ませた提案(E6 通過を仮定した残留インジェクション)。期待終端状態(released ×2, rejected ×3)と、SPEC/PR/図鑑データの内容を目視確認。CX-05 の「開発者の操作 2 点以内」実績はここで記録する(2026-08-11 改訂: 有効化はマージ後の自動連鎖へ移行 — §2.6)。
...
1. **§4.7「常駐ワーカーによる自動パイプライン」→ 人間承認駆動へ改訂**: 判定(E6 L3 + CX-01)はローカル判定ツール(codex app-server)、実装は開発者マシンの skill、マージは開発者(有効化はマージ後に skill が自動実行 — 2026-08-11 §2.6)。auto-merge は使わない。「人手ゼロで閉じる」の記述(§4.7-6)も改訂対象(E-17)。あわせて §7-2(A-4)は解消: codex は開発者の対話的セッションから起動するため、ヘッドレス常用の規約問題は生じない(全自動化を再検討する場合に再燃)。
