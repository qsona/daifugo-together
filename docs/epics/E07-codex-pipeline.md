...
- **運用方針(2026-08-29実装、decision-log C-14)**: 全量 simulation は release push の CI で実行する。前回 release から変更された各ルールの `new-only` と全ルール同時有効の `all-rules` を各 200 ゲーム × 5 seed で検証し、成功するまでデプロイしない。ルール PR は `new-only` を 20 ゲーム × 1 seed の smoke とし、全ルール共存検証は共有 CI の速度揺れによる誤検知を避けて release ゲートで行う。diff-guard・quality・rule-testsは維持する。
