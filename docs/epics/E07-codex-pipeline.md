...
| E6(インジェクション対策) | 受け | YC-01 の検査を**通過した提案だけ**がパイプラインに入る。検査は提案送信時に同期実行。遮断された提案は本 Epic に一切届かない |
...
|---|---|---|
| 審査中 | `screening` | 可否判断待ち・判断中・**開発者の確定待ち**(却下推薦・needs_review を含む) |
| 却下 | `rejected` | 実装不可と確定(初期は開発者確定後。§3.1)。理由区分つき |
...
sequenceDiagram
    ...
    participant C as codex(隔離実行)
    participant G as GitHub(PR/CI)

    U->>S: 提案送信(E5)
    S->>S: インジェクション検査(E6・同期)
    S->>S: proposals 保存(screening)
    S->>S: CX-01 可否判断(サーバー内ジョブ・LLM API)
    alt 却下推薦 / needs_review
        S->>D: 開発者へ通知 → admin で確定(却下なら RP-03 表示)
    else 実装可
        S->>S: judgement + SPEC 記録 → queued
        D->>S: 実装skill 起動 → 次のジョブを取得(admin API)
        D->>G: ブランチ作成 + scaffold コミット & push(meta.json / SPEC.json)
        D->>C: codex exec(隔離ワークスペース)
...
| # | 段階 | 担い手 | 入力 | 出力 | 失敗時の挙動 |
|---|---|---|---|---|---|
| 1 | 受付・検査 | server(E5/E6) | 提案フォーム | `proposals`(screening) | 検査遮断 → イエローカード(E6)。パイプラインに入らない |
| 2 | 可否判断(CX-01) | server 内ジョブ + LLM API | 提案本文(E6 の sanitized)・既存ルール一覧・線引き基準 | `judgements` 行 + SPEC。approve → `queued` / 却下推薦・needs_review → 開発者確定待ち | LLM API 障害は最大 3 回再試行(バックオフ)→ なお不可なら保留し、以後の定期スイープで自動再試行(提案は失わない) |
| 3 | codex 実行(CX-02) | 実装 skill(開発マシン)+ 隔離実行 | scaffold 済みワークスペース + プロンプト | ルールディレクトリの生成物 | タイムアウト/異常終了/検収 NG → その場で開発者が再実行(1 回目安)か打ち切り(`failed`)を判断 |
...
旧版の「別ホストの常駐ワーカー」は採らない。**パイプラインの各段をライブラリ関数として実装し、薄いドライバ 2 つで駆動する**:

| ドライバ | 置き場所 | 担当 | 起動契機 |
|---|---|---|---|
| **judge ジョブ** | サーバープロセス内 | CX-01(可否判断 + SPEC 生成) | 提案保存時に非同期キック + 定期スイープ(取りこぼし・API 障害からの回復用) |
| **実装 skill** | 開発者マシン(claude / codex CLI の skill) | CX-02〜03 の駆動(scaffold → codex → 検収 → PR → CI 監視)と、レビュー・マージ・有効化の支援 | 開発者が手動起動 |

設計上のポイント:

- **judge をサーバー内に置く理由**: E6 の検査(同期・LLM 使用)が既にサーバーで動くため、LLM クライアント(`packages/server` 内の薄い共通クライアント)と API キーの置き場所が 1 か所で済む。判定は安価・副作用小(DB 書き込みのみ)なので無人実行してよい。codex のような重い実行はサーバーに載せない。
...
- **skill とサーバーの通信**: サーバーが admin API(HTTPS + Bearer トークン)を提供し、skill はこれ経由で読み書きする(SQLite の単一ライタをサーバープロセスに限定する方針は維持)。主なエンドポイント: `GET /admin/pipeline/next`(次の queued ジョブの払い出し)/ `POST /admin/pipeline/jobs/{id}/update`(phase・branch・pr_number 等の記録)/ `POST /admin/pipeline/jobs/{id}/fail` / `POST /admin/proposals/{id}/judge`(却下・needs_review の確定)/ `POST /admin/rules/{id}/enable | disable`。
- **E6 G2 の実装点**: `GET /admin/pipeline/next` のハンドラは払い出し前に、当該提案に `finalVerdict='pass'` の検査記録(E6 `proposal_checks`)が存在することを再確認し、なければ払い出さず開発者アラートを出す。応答には確認済みの `passedCheckId` を含め、skill はこれを欠くジョブを処理しない。
...
- codex 実行は人間駆動のため頻度上限を持たない(自然律速)。デプロイ時間帯への配慮(E-15: 遊ばれている時間帯を避ける)も、開発者がマージのタイミングで自然に行う。
- 可否判断(LLM API)のみコスト保護のソフト上限を置く: `JUDGE_RATE_PER_DAY`(暫定 200 件)。超過分は翌日のスイープに回る。
...
| `JUDGE_RATE_PER_DAY` | 200 |
| `REJECT_CONFIRMATION` | `manual`(却下は開発者確定。§3.1。判定品質に自信がついたら `auto` へ) |
| codex 隔離実行の資源上限 | `--memory 2g --cpus 2`(docker 利用時) |
...
5. **実行の隔離**: codex の実行は開発マシン上でも隔離して行う——docker コンテナ(git 認証情報・サーバーの秘密情報は渡さない。push・PR 作成はコンテナ終了後に skill が行う)、または codex CLI 自身のサンドボックス機構。**どちらを最低線とするかは C-7 で決める**(§5.1-5)。開発者が画面の前にいるとはいえ、エージェントの逸脱行動は目視より速い前提で設計する。
...
- 入口: E6 の検査を通過して保存された提案(`screening`)。サーバー内の judge ジョブが処理する(提案保存時にキック + 定期スイープ)。
- 判断 AI は 1 回の LLM API 呼び出し(構造化出力)で、**可否判定と仕様正規化を同時に**行う。正規化出力(SPEC)が CX-02 の入力になる。
- verdict は 3 値: `approve` / `reject` / `needs_review`。
  - `approve` → SPEC を保存し `queued` へ(自動。人間確定は不要 — 実装後の PR レビューが人間の関門になるため)。
  - `reject` → **初期は AI の推薦に留め、開発者が admin で確定してから** `rejected` + RP-03 表示(`REJECT_CONFIRMATION=manual`)。人間のレビューなしにユーザー向け通知が出る唯一の経路を初期は塞ぐ趣旨。判定品質が確認できたら `auto` に切り替え可能(切り替え判断の材料は §4-5 の評価セットと stage0-issue の実績)。
  - `needs_review`(確信度が低い・線引きの境界)→ 開発者が admin で approve/reject を確定。表示上は「審査中」のまま。
- 失敗系: LLM API 障害 → 3 回再試行 → 保留(定期スイープで自動再試行、開発者アラート)。出力がスキーマ不正/検証不合格 → 1 回だけ再呼び出し → なお不正なら needs_review 扱い。
...
- API: `POST /admin/proposals/{id}/judge`(却下推薦・needs_review の人手確定)。judge ジョブはサーバー内なので内部 API は不要(直接サービス層を呼ぶ)。
- 判断 AI の実装方式: **従量課金の小型 LLM API**(1 提案 1 円未満のオーダー。codex の subscription 枠は実装専用に温存する)。プロバイダ・モデルは E6 のインジェクション検査と共通の薄いクライアント越しに使い、選定は E6 と合同で行う(C-2。基準: 構造化出力対応・日本語・低コスト・応答 60 秒以内)。temperature 0 相当の決定的設定を使う。
...
**(d) 実装方針** — サーバー内モジュール(`packages/server` の pipeline サービス層 `judge.ts`)。LLM クライアントは E6 と共用。既存ルール一覧は同一プロセス内のレジストリ + DB から取得。プロンプトと線引き表はファイルで管理し、変更は PR で追跡。
...
**(f) 未解決事項** — LLM プロバイダ選定(C-2)/ B-1(choice 機構)の正式決定で A1 行が変わる / `REJECT_CONFIRMATION` を auto に切り替える判断基準の明文化(運用実績後)。
...
- 従量課金 LLM を使わない: 実装工程では codex(subscription)のみを使う。可否判断(§3.1)の小型 LLM は「実装処理」ではなく審査であり、企画書 §9-8 が想定済みのコスト。この整理を受け入れ条件の解釈として明記する。
...
- GitHub 操作: skill は開発者のローカル git / `gh` 認証をそのまま使う(開発者本人の操作の延長。専用トークンの発行・管理は不要)。codex の隔離実行環境には git 認証情報を渡さない(§2.4-5)。

**(d) 実装方針** — パイプライン段の実装は `packages/pipeline` のライブラリ(`workspace.ts` / `codexRunner.ts` / `inspector.ts` / `github.ts`)。skill はこれを呼ぶ薄い CLI(`pnpm pipeline implement` 等)+ skill 定義ファイル。git 操作はホスト側で CLI をラップ。codex CLI のバージョン固定と隔離方式は C-7 の決定に従う。結合テストは FakeCodexRunner で codex 非依存に行う(§4)。

**(e) 受け入れ条件の精緻化**

- approve 済み提案から、**skill の起動操作以外の人手なしに** PR 作成まで到達する。
- 実装工程から従量課金 LLM API のキーに到達できない(判定用キーはサーバーにのみ置き、開発マシン・codex 実行環境には配置しない)。
- 成否・失敗コード・試行回数が `pipeline_jobs` で提案に紐づいて参照できる(OP-02 の集計元)。
- skill のセッションを任意の段階で強制中断しても、再起動後に二重 PR・二重ブランチが生じない(§4 のテストで確認)。

**(f) 未解決事項** — codex 実行の隔離水準(C-7。§5.1-5)。
...
5. **判定品質の評価セット(CX-01)**
   - 線引き基準表(§3.1)の各行につき最低 1 件、計 20〜30 件の提案文と期待 verdict を評価セットとして保持。プロンプトやモデルを変えるたびに一致率を測る(スクリプト 1 本)。`REJECT_CONFIRMATION` を auto へ切り替える判断材料もこの一致率(特に reject の精度)。
...
1. **可否判断 LLM のプロバイダ・モデル選定**(C-2): E6(インジェクション検査)と共通クライアントで使うため、E6 と合同で選定する。基準: 構造化出力対応・日本語性能・1 提案 1 円未満・応答 60 秒以内。
...
5. **codex 実行の隔離水準(C-7 の再定義)**: 実行場所が開発者マシンになったため、リスクの形が変わった——最悪ケースは「開発マシン上のエージェントが敵対的入力に汚染され、ローカルの資格情報・ファイルへ到達する」こと。選択肢: (i) docker コンテナ実行(旧設計を流用: workspace のみマウント、git 認証・サーバー秘密は渡さない、必要なら egress allowlist)、(ii) codex CLI 自身のサンドボックス機構(ファイルシステム・ネットワーク制限)で足りるか実測して判断。**運用開始前に決定する**。あわせて既知の残存リスクとして記録: 悪意あるルールが発動パターンや Effect パラメータの選択を通じて秘匿情報を低帯域でリークする経路(covert channel)は、差分ガードでは原理的に塞げない。**人間レビュー(§2.7 チェックリスト 3)が恒久的な検出点**であり、これは本改訂(人間承認駆動)で旧版より強化された点である。
6. **RP-03 の期待値文言**: 反映まで数時間〜数日かかる前提の表示・通知文言(E5 と調整。§2.1)。

### 5.2 E12 への修正提案

1. **§4.7「常駐ワーカーによる自動パイプライン」→ 人間承認駆動へ改訂**: judge はサーバー内ジョブ、実装は開発者マシンの skill、マージ・有効化は開発者。auto-merge は使わない。「人手ゼロで閉じる」の記述(§4.7-6)も改訂対象(E-17)。あわせて §7-2(A-4)は解消: codex は開発者の対話的セッションから起動するため、ヘッドレス常用の規約問題は生じない(全自動化を再検討する場合に再燃)。
...
3. **RP-03 の期待値文言**(§5.1-6): 「審査中」「実装中」が数時間〜数日続く前提の文言・通知設計を E5 側で調整。
