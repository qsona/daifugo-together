...
> - **通知方式の後日判断(§3.3(g))は解消(2026-07-29、decision-log G-15)**: 案 B(通知センター)は E16 として採用、案 C(Web Push)は E15 + E16 計測後の投資判断ゲート付きで E17 として採用。案 A(未読バッジ)は置換されず E16 と併存する(E16 §1.2 NC-D2)。
...
- **「出自の記録」という意味論のデータへの反映**(§4.1)。この列は「提案者がそのルールを遊んでいたと自己申告した都道府県」であり、「そのルールが実在する地域」ではない。E5 はこの意味を **データ層の規約として固定**し、下流に契約として渡す:
  ...
  - 表示は提案フォームと提案者本人のマイ提案に限定する。ルール図鑑・対局画面・運営の成功指標には使わない。

エッジケース:

| ケース | 挙動 |
|---|---|
| **空都道府県(ローカル・未選択)** | 許可。`prefecture_code=NULL`。マイ提案では「ローカル(県の記載なし)」表示 |
...
- **都道府県マスタ**: JIS X 0401 に基づく固定 47 件。DB テーブルではなく **TS 定数**として持つ(不変の参照データ。過剰設計回避)。
...
- 意味論(本人向けの記録)は本書と `rules` へ引き継ぐフィールドで明示し、公開 UI や成功指標へ転用しない。
