...
| WP-D4 | センターとの関係 | **Push 対象は終端結果 3 種(`proposal_released` / `proposal_rejected` / `proposal_failed`)に固定**(2026-08-02 改訂)。何を Push するかの正は E16 種別レジストリのチャネル属性(E16 §2.2)のみ。**Push は常にセンターの複製**であり、Push でしか得られない情報を作らない(許諾しない・できない子供が不利にならない)。旧仕様の「種別ごとに ON/OFF 可能」は廃止した — 設定トグルの区分(「確認結果」と「実装結果」)はユーザーに区別不能で、オプトインの約束「結果が出たら通知」は実装失敗を含む結果全部を指すため |
...
- 送信条件(2026-08-02 改訂): 種別チャネルが「センター+Push」∧ 夜間でない ∧ 有効な購読が 1 件以上。種別ごとのユーザー設定は参照しない(廃止 — §2.3)。現時点で「センター+Push」チャネルは終端結果 3 種(`proposal_released` / `proposal_rejected` / `proposal_failed`)であり、購読 = この 3 種を受け取ることがオプトインの約束になる。予約種別(`friend_invite`・`stats_milestone`)が実装されれば、E16 レジストリのチャネル属性に従い自動で送信対象に乗る(E17 側の追加実装は不要)。
...
- 単体: 送信条件の全分岐(チャネル外種別は送らない・夜間は送らない・失効購読は送らない・購読があれば「センター+Push」3 種すべて送る)、夜間境界(20:59/21:00/6:59/7:00 JST)、410 での失効、endpoint upsert の冪等性、403 ゲート。
...
- [ ] 購読 = 終端結果 3 種の受信であり、種別ごとの選択 UI・API は存在しない。設定画面に残るのは「この端末で通知を受け取る」(購読)と「この端末への通知を止める」(解除)だけ
- [ ] ログアウトで購読が解除される
