...
- 作成日: 2026-07-29
- 状態: 実装済み(2026-08-01。WP-01〜03。VAPID 設定と実機受入は runbook の実施待ち)。**2026-08-02 改訂の差分実装待ち**(`docs/specs/2026-08-02-push-optin-simplification-design.md` 参照)
...
>
> **2026-08-02 改訂(Push オプトイン簡素化。決定 G-24)**: 開発者が以下を裁定した。差分実装の作業リストは `docs/specs/2026-08-02-push-optin-simplification-design.md` が正。
>
> 1. **Push 種別の選択 UI を廃止し、Push 対象を終端結果 3 種(`proposal_released` / `proposal_rejected` / `proposal_failed`)に固定する**。設定画面のトグル(「提案の確認結果が出た」「ルールの実装結果が出た」)はユーザーに区別不能で、オプトインの約束「結果が出たら通知」には実装失敗も含まれる。`proposal_implementing`・`rule_debut` はセンターのみ(現状どおり)。
> 2. **`push_preferences`(テーブル・API・設定画面のトグル)を撤去**する。E16 種別レジストリのチャネル属性(E16 §2.2)を唯一の真実とする。設定画面に残るのは「この端末への通知を止める」(購読解除)のみ。
> 3. **オプトイン動線の強化**: 提案送信直後のオファー(登録済みユーザー向け)は維持しつつ、**匿名おためし提案の送信直後にアカウント登録を強めに促す導線**を追加し、E15 ログイン完了の戻りで Push オファーへ継続する。
> 4. **再提示ポリシーの緩和**: 「提示は 1 回のみ」を改め、明示的に「受け取らない」を選んだ端末を除き、次回以降の提案送信時にも再オファーしてよい。
>
> 本文の WP-D3 / WP-D4・§2.1・§2.3〜2.6・§2.10・§3(WP-02/03)は改訂済み。§4 の差分リストは 2026-08-01 実装時の記録であり、改訂の差分は上記 spec を参照。
>
> **2026-08-03 拡張(管理画面のお知らせ配信)**: `announcement` を「センター+Push」種別に追加した。購読者の種別選択を復活させる変更ではなく、購読中の端末には提案終端結果 3 種に加えて運営のお知らせも届く。夜間抑止・同一文面源・センターが正という既存原則は維持する。
...
| WP-D3 | オプトイン | **文脈付きオプトイン**。初回訪問時に許諾 UI を一切出さない。**提案送信直後**に「結果が出たら通知を受け取る?」を提示し、承諾した場合のみブラウザの許諾ダイアログへ進む(提案しない限り Push 許諾 UI は存在しない)。**再提示(2026-08-02 改訂)**: 明示的に「受け取らない」を選んだ端末には二度と出さないが、それ以外の未購読端末には**次回以降の提案送信時にも再オファーしてよい**(提案送信という文脈がある限り煩くない。iOS のホーム画面追加で一度離脱した人の回収経路)。加えて、**匿名おためし提案の送信直後は登録を強めに促し、E15 ログイン完了の戻りで Push オファーへ継続する**(§2.4 末尾。匿名のまま購読させるのではない — WP-D2 は不変) |
| WP-D4 | センターとの関係 | **Push 対象は種別レジストリの「センター+Push」種別**。2026-08-02 時点の終端結果 3 種(`proposal_released` / `proposal_rejected` / `proposal_failed`)に、2026-08-03 から `announcement` を追加した。何を Push するかの正は E16 種別レジストリのチャネル属性(E16 §2.2)のみ。**Push は常にセンターの複製**であり、Push でしか得られない情報を作らない(許諾しない・できない子供が不利にならない)。種別ごとの ON/OFF は設けない |
...
| WP-02 | Push 購読と文脈付きオプトイン(購読 API + 提案送信直後の提示 + 購読解除の設定。2026-08-02 改訂で「種別ごと設定」を廃止) | WP-01・E15・E16 |
...
- **せがまない(WP-D3 の維持。2026-08-02 改訂)**: 提示は提案送信直後に限る(それ以外の場面では出さない)。「今後は出さない」を選んだ端末には追加案内も出さないが、単に閉じただけの端末には**次回以降の提案送信時に再提示してよい**(WP-D3 の再提示緩和と同じ整理。G-23 の「1 回のみ」はこの点のみ改める)。手順は通知設定画面から常時たどれるため、閉じても失われない。
...
CREATE TABLE IF NOT EXISTS push_subscriptions (
  ...
  revoked_at     INTEGER                 -- NULL=有効。失効(410 等)・ユーザーの購読解除で設定
);
...
- **`push_preferences` テーブルは廃止**(2026-08-02 改訂)。種別ごとの ON/OFF はユーザーに区別不能な選択を強いるだけだったため撤去し、「何を Push するか」は E16 種別レジストリのチャネル属性のみで決まる。購読者ゼロの時点での撤去のため移行は不要(起動時マイグレーションで `DROP TABLE IF EXISTS`。spec 参照)。ユーザーが選べるのは「この端末で受け取るか」(購読の有無)だけになる。
...
| ~~`GET`/`PUT /api/push/preferences`~~ | **廃止**(2026-08-02 改訂。§2.3。購読=送信対象であり、種別選択は存在しない) |
...
**オファー動線(2026-08-02 改訂)**:

- **登録済みユーザー**: 提案送信成功の直後にオファー(購読提示、iOS の未追加端末ではホーム画面追加案内)。未購読かつ「受け取らない」を選んでいない端末には、送信のたびに再提示してよい(WP-D3)。
- **匿名ユーザー**: 購読は提示しない(WP-D2)。代わりに、おためし提案(`docs/specs/2026-07-30-anonymous-trial-proposal-design.md`)の送信成功直後に**登録を強めに促す**——「けっかが出たらお知らせできるよ。Google で引き継ぎ登録すると通知が受け取れる」(文言は仮)。承諾すると E15 のログイン開始(begin)へ進み、**ログイン完了の戻り(E15 §2.7 の完了ハンドラ)で Push オファーに継続する**。提案直後の熱量を登録インセンティブに接続する狙い。継続の実装形(フルページ遷移をまたぐフラグの持ち方)は spec 参照。
...
- 送信条件(2026-08-03 更新): 種別チャネルが「センター+Push」∧ 夜間でない ∧ 有効な購読が 1 件以上。種別ごとのユーザー設定は参照しない(廃止 — §2.3)。現時点の対象は終端結果 3 種(`proposal_released` / `proposal_rejected` / `proposal_failed`)と管理画面のお知らせ(`announcement`)。予約種別(`friend_invite`・`stats_milestone`)が実装されれば、E16 レジストリのチャネル属性に従い自動で送信対象に乗る(E17 側の追加実装は不要)。
...
- 未設定なら Push 機能だけ無効。役割別に確定する: **`GET /api/push/config` は 200 + `{ available: false }`**(クライアントの導線出し分け用の照会であり、エラーではない)。**購読系 API(`POST`/`DELETE /api/push/subscriptions`)は 503 `{ error: 'push_unavailable' }`**(E15 §2.9 の `auth_unavailable` 503 と同じ縮退の流儀)。開発・テストは `web-push` をモックした `FakePushTransport`(§2.10)で実クレデンシャルなしに完了できる。
...
| ケース | 挙動 |
|---|---|
| ブラウザ許諾で「拒否」 | 購読せず終了。センターで全情報が得られる旨を一言表示。ブラウザ許諾が `denied` の端末には再提示しない(WP-D3 の再提示緩和はアプリ内オファーの話で、ブラウザに拒否された端末は購読不能。設定画面からはやり直せる) |
...
- 単体: 送信条件の全分岐(チャネル外種別は送らない・夜間は送らない・失効購読は送らない・購読があれば「センター+Push」種別を送る)、夜間境界(20:59/21:00/6:59/7:00 JST)、410 での失効、endpoint upsert の冪等性、403 ゲート。
- E16 結合: `publish()` 1 回でセンター INSERT + Push 送信が同一ペイロード源から出ること(文面の二重定義がないこと)。
- クライアント: オプトイン提示の条件(提案送信直後・登録済み・未購読・「受け取らない」未選択のときだけ。再送信時の再提示と、「受け取らない」後は出ないこと)、匿名送信直後の登録導線とログイン戻りでのオファー継続、`?src=push&nid=<notificationId>` 検出→ opened 記録→パラメータ除去(§2.7)、設定画面の購読・解除。
- 実ブラウザでの通し確認(デスクトップ Chrome/Firefox + iOS 実機のホーム画面追加)は WP-T1/T2 完了後に手動で行い、結果は `docs/status.md` の該当項目を更新。
...
### WP-02: Push 購読と文脈付きオプトイン(2026-08-02 改訂)

「提案者として、提案を送った流れのなかで『結果が出たら知らせて』を選びたい。それ以外の場面で許諾をせがまれたくない」

受け入れ条件:

- [ ] 初回訪問・通常プレイには Push 許諾 UI が一切出ない。匿名ユーザーには購読 UI を出さない(登録導線のみ。下記)
- [ ] 登録済みユーザーの提案送信直後にだけ「結果が出たら通知を受け取る?」が提示され、承諾時のみブラウザ許諾→購読登録が走る(拒否してもセンターで全情報が得られる)。「受け取らない」を選んだ端末には以後出さないが、それ以外の未購読端末には次回の提案送信時に再提示される
- [ ] 匿名ユーザーのおためし提案送信直後に登録を促す導線が出て、ログイン完了の戻りで Push オファーに継続する
- [ ] 購読は `push_subscriptions` に保存され、匿名ユーザーの購読 API は 403 `registration_required`
- [ ] 購読 = 「センター+Push」種別(終端結果 3 種と管理画面のお知らせ)の受信であり、種別ごとの選択 UI・API は存在しない。設定画面に残るのは「この端末で通知を受け取る」(購読)と「この端末への通知を止める」(解除)だけ
- [ ] ログアウトで購読が解除される
...
2026-07-29 に実コードを確認した現状に基づく(E16 §4 の差分適用後を前提とする)。**この表は 2026-08-01 実装時の作業リストの記録である**。2026-08-02 改訂(`push_preferences` 撤去・種別 ON/OFF 廃止・オファー動線強化)の差分作業リストは `docs/specs/2026-08-02-push-optin-simplification-design.md` が正。下表の `push_preferences`・「種別 ON/OFF」への言及は改訂で撤去対象になった(注記付き)。
...
| 差分-2 | `packages/server/src/persistence.ts` | E16 差分-1 適用後: `notifications` あり | `push_subscriptions`・`push_preferences` を既存マイグレーション方式で追加。`PushRepository` 新設(※ `push_preferences` は 2026-08-02 改訂で撤去対象) |
...
| 差分-6 | `packages/web/src/screens/ProposalFormScreen.tsx` ほか | 提案送信成功→マイ提案へ遷移 | 送信成功後のオプトイン提示(登録済み・未購読のときのみ。E15 の未登録分岐と共存)。設定画面(種別 ON/OFF・「この端末への通知を止める」)の追加(※ 種別 ON/OFF トグルは 2026-08-02 改訂で撤去対象) |
...
| マイ提案画面の未購読者向けインライン購読バナー | 2026-08-02 改訂のスコープ外。後続候補として記録のみ(裁定 5) |
...
| WP-T4 | オプトイン提示の文言と再提示ポリシー(拒否後に設定画面以外で再び誘うか) | **2026-08-02 裁定済み**: 「受け取らない」を明示した端末以外には、次回以降の提案送信時に再提示してよい(WP-D3)。文言は UI 文言ガイド準拠で仮置き→レビュー | 裁定済み |
