...
- **手番の流れ**: サーバーのゲームループが AI の手番を検知すると、`packages/ai` の `decideMove` を呼ぶ。ライブラリ既定は E12 予算(計算 50ms 平常・200ms 上限)を保ち、全ルールを使う本番ルームは初手を含む65手の探索用に上限600msを渡す。**演出遅延**(0.4〜1.2 秒のランダム。値は E4 のトーンと合わせて調整)を挟んで着手し、人間から見て「即打ちでも長考でもない」テンポを作る。
...
- **プレイアウト**: 決定化した世界でルート候補手を 1 手打ち、以後は全員がロールアウト方策(下記)で、初手を含む `cutoffSteps`(既定 65 手)まで、または終局まで進める。
...
- **フォールバック階段**(上から順に試し、必ず着手を返す):
  1. 探索完走 → 訪問数最大の手
  2. ハード期限超過(ライブラリ既定200ms、本番ルーム600ms) → その時点の統計で最善の手(反復 0 なら次へ)
...
export interface MctsConfig {
  ...
  cutoffSteps: number;     // 既定 65。ルート候補手を含む
...
5. **時間内**: 1 手の計算が指定したhardMs(ライブラリ既定200ms、本番ルーム600ms)+ マージン以内。応答不能は1秒watchdogで打ち切る。
...
| 項目 | 目標 |
|---|---|
| AI の計算時間 | ライブラリ既定は平常 ≤ 50ms・上限200ms。本番ルームは65手探索のため上限600ms。超過はフォールバック |
