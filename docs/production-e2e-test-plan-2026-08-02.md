...
6. **3-6. 起動時 revert 同期の実確認**: 本番 `bin.ts` が静的 registry と接続され起動時同期を行うか(`docs/archive/impl-progress.md:994` の記述が stale か)をログで確認し、結果は `docs/status.md` の該当項目を更新する
...
3. **4-3. セーフエリア実機確認**: ノッチ / ホームインジケータにボタン・手札が隠れないこと(`docs/archive/impl-progress.md:1157`。ブラウザでは原理的に検証不能、実機必須)。提案フォームの横スクロールなし・キーボード表示中に送信ボタンが押せること
...
3. **5-3. 匿名おためし提案枠**: 匿名で提案 1 件送信 → 2 件目が枠 403 パネル → 登録導線(`docs/archive/impl-progress.md:6` の実画面未確認の消化)
...
3. **6-3. 受け入れ記録**: 実施結果は対応する runbook 末尾の「実施記録」節へ追記し(節がなければ作る)、`docs/status.md` の該当行を更新する(完了なら行を削除する)。secret・トークン類は書かない
