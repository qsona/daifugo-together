...
> このファイルはアーカイブです(2026-08-11 移設)。
> 現行フローは [../implementation-guide.md](../implementation-guide.md)、現在の残件は [../status.md](../status.md) を参照してください。
> 本書の記録は 2026-08-02(`13d71fe`)で停止しており、それ以降の作業は記録されていない。一部の記述(例: E11 の都道府県カバー共有集計)は後続の実装で陳腐化している。

## 現在

- **認証・アカウント UI 改善(2026-08-02) 実装・検証完了**: メニューのアカウント行、記録/なまえ画面、共通の接続・切替・サインアウトダイアログ、アプリ内で完結するOAuth結果表示、退室後/提案/通知設定の入口と語彙統一を実装した。サーバー契約は変更していない
...
- **「あそぶ」導線の再編(2026-08-01)**: 選択の軸を「だれとあそぶか」へ組み替え(2段ウィザード)、きほんを1人練習専用(`ROOM_SOLO_ONLY` + 待機室なしの自動開始)にした。「はじめてのひとはこちら」タグと未プレイ端末による並べ替えは廃止。**素の大富豪の人間マルチを提供しなくなるトレードオフを受容**。記録はE14章末、裁定は決定台帳 G-19 / G-20
- **フェーズ 2 / E14 TU-01〜04 プロセス2完了・完了レビューPASS**: きほんの部屋、カード案内、初戦ガイド、初回卒業導線まで実装・自動テスト・375×812実画面確認を完了。独立完了レビューのImportant 1件を修正し、最新`main` (`1b15732`) 基点へ統合・全検証・独立再レビュー済み(導線部分は上記の再編で更新)
...
> **2026-08-01 追記** — 本節の導線に関する記述(モード2択の ChoiceSheet、「はじめてのひとはこちら」タグ、きほんの人間マルチ)は当時の実装の記録である。導線は本章末の「「あそぶ」導線の再編(2026-08-01)」で差し替えられており、**現在の正はそちら**。サーバー契約(`RoomMode`・ルール解決のモードゲート)は本節のまま有効。
...
> **2026-08-01 追記** — 本節に残る「きほん人間2人」「人間マルチ」の分岐は、導線の再編できほんが 1 人固定になった今も**到達しない安全側の分岐として実装に残している**もので、誤りではない。導線の現在の正は本章末の「「あそぶ」導線の再編(2026-08-01)」。
...
### 「あそぶ」導線の再編(2026-08-01)

TU-01 で入れた「モード2択の ChoiceSheet + 未プレイ端末のタグ」を差し替える決定。上記 TU-01 の記録は当時の実装として残し、**現在の正は本節**とする(決定台帳 G-19 / G-20、チュートリアル設計 §3、E14 §2.1)。

#### 決めたこと

- **軸を「だれとあそぶか」へ**。`PlaySheet` を2段のウィザードにした。1段目「みんなのルールであそぶ」(primary・先頭)/「ひとりで練習する」(若葉アイコン + キャプション「大富豪がはじめての人はこちら」)、2段目「部屋を立てる」(primary)/「友だちの部屋にはいる」/「もどる」
- **「はじめてのひとはこちら」タグと未プレイ端末による並べ替えは実装しない**。並びは常に同じで、常設キャプションが端末状態に依存しない道案内を果たす。`daifugo.playedBefore` の用途はガイド文言と強さ目盛りの出し分けだけに縮んだ
- **きほんは1人練習専用**。`RoomManager.join` が `room.mode === 'basic'` を `ROOM_SOLO_ONLY` で拒否する(`ErrorCode` に追加)。Web の文言は「この部屋はひとりで練習する部屋です。友だちの部屋の招待コードをたしかめてください。」
- **きほんは待機室を挟まない**。`createRoom('basic')` 成功後にそのまま `startRoom()` を呼ぶ。開始に失敗して待機室に残った場合も `showInvite` を偽にして招待コードを出さない(部屋の主は自分ひとりなので専用の文言は置かない)
- 旧ボタン文言「きほんルールであそぶ」は廃止。「友だちの部屋にはいる」はラベルを保ったまま2段目へ移した

#### 受け入れたトレードオフ

- **素の大富豪(有効ルール0)の人間マルチは提供しなくなった**。企画書 §3.5 とチュートリアル設計 §0 が想定していた「卒業後も家族で遊ぶ場」のユースを落とす判断であり、記録として残す。人間どうしで遊ぶ場は「みんなのルール」だけになる
- 埋め戻す余地: 将来「だれかとあそぶ」(公開部屋 + AI 埋めのドロップイン)を足すとき、または実需が出たときに再検討する。トップの軸を「だれと」にしたのは、この拡張を同じ階層に足せるようにするため(フェーズ2で公開部屋、フェーズ3でマッチング — 方向性であって決定ではない)

#### 解消した未決事項

- **C-10(マルチプレイ時のモード互換)= 解消**。入室前にモードを見せる仕組みを作る代わりに、きほんへの入室自体を拒否して理由を文言で伝える形にした。E14 §5 のリスク3も同時に閉じた
...
### 認証・アカウント UI 改善（2026-08-02）

- メニュー上部を共通のアカウント行へ置き換え、未登録・登録済み・認証確認中・接続中の4状態を表現した。通知設定は同じ行へ残し、記録画面からGoogle接続、サインアウト、なまえ変更へ進める
- OAuth開始・完了はページ遷移やブラウザ標準ダイアログに依存せず、共通ダイアログ DLG1 / DLG2 / DLG3 とルートレベルの結果表示で完結させた。beginの401は匿名sessionへ切り替えて一度だけ再試行し、503は利用不可として案内する
- 提案フォーム、通知設定、退室後結果、招待コード、インストール案内の入口と文言を整理し、「Googleでつなぐ」「サインアウト」「わるい命令」へ語彙を統一した。表示名入力は共通化し、Unicodeコードポイント10文字と制御文字禁止をクライアントでも検証する
- OAuthのフルリダイレクトをまたいで「登録済みだったか」を判定する必要があるため、その一時状態だけは`sessionStorage`へ保持する。保存不能でも認証処理を止めない
- 共通schemaが`trim()`後に検証するため、末尾タブなどを黙って受理しないよう、生の入力に対する制御文字検査を先に行う。サーバー契約とschema自体は変更していない
- 招待文言の「だれでも」は招待リンクを知る人を指す表現として採用した。Q7は「あなた」の正準表示関数への集約まで実施し、待機画面の表示形式変更は別途裁定が必要なため含めていない
- 完走セット後の接続案内は、未登録端末に対して3セットごとに表示する。表示済みセットIDと回数は安全な端末内storageへ保存し、中断セットや同一セットの再表示では加算しない
...
- 都道府県は提案フォーム、マイ提案、保存データ、ルールメタデータ、公開APIレスポンスに残す。図鑑の表示・絞り込み・カバー集計、対局内ルール詳細、判断AIの入力、運営の成功指標、告知には使わない
- 図鑑は状態・区分・並び順だけで絞り込み、ルールカードと詳細は `ローカル` / `オリジナル` の区分とルール説明を表示する。都道府県を持つAPI応答でもDOMへ県名を出さない回帰を追加した
- `prefectureCoverage` と共有集計関数を catalog / ops metrics から削除し、CX-01プロンプトから `prefectureCode` を除外した。判断入力の変更に伴いプロンプト版を `cx01-v9` へ更新した
- 企画書、バックログ、E05/E07/E10/E11、運用runbook、デザインシステム、ワイヤーフレーム、X告知計画を新しい表示範囲へ合わせた。decision-log G-21を正とする
