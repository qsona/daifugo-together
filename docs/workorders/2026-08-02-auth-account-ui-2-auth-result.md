...
- 現行の `App.tsx:1082-1084`(`linked` かどうかだけで分岐し `switched` と `already` を「おかえり!」にまとめる)を、**3 outcome の区別**に直す(J5-1)

### 3.5 RES-2: `switched` = ダイアログ 2 種(Q-8)
...
- 「もう一度ためす」は既存表記(`RuleDexScreen.tsx:174`)に揃えた語
