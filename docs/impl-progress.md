...
- Epic: E4 / 周: プロセス2 完了 / ストーリー: DS-01・DS-02 / 次にやること: プロセス2 のレビュー(完了レビュー)。承認後に main へチェックイン
...
| DS-01 / DS-02 | プロセス2 | この行を含むコミット | `pnpm verify` 成功(format:check / lint / **lint:design** / typecheck / **30 tests** / 6 packages build)。ブラウザ実描画で 6 画面 × 3 ブレークポイントを確認 |
...
5. **キービジュアル 2A に JOKER カードが描かれているが、素の大富豪 v1(decision-log A-3、2026-07-25 決定)は「ジョーカーを初期から除外」としている。** 2A の決定は 2026-07-24 で A-3 より前。原資産は勝手に変更しない契約なので報告に留める。**OGP 画像も 2A から生成しているので同じ食い違いを含む**(共有先で最初に目に入る絵)。
6. **ロゴのアウトライン化(E04 §5-1)を実施できなかった。** 実グリフをパス化するには M PLUS Rounded 1c の非圧縮フォントファイルとフォントツールキット(fontTools 等)が要り、同梱している woff2 は unicode-range でサブセット分割・圧縮されていて直接は使えない。加えて、パス化の忠実度の合否は人間の目視でしか判定できない(プロセス1 レビューの裁定も「実質: 要人間判断」)。**実施するかどうかと、実施する場合の元フォントの入手方法の判断をお願いしたい。** 現状はライブテキスト版のままで、Android/Windows ではロゴの字形が変わる。
