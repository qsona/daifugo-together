...
- **過負荷時の挙動は degrade であってハングではない**: `packages/server/src/ai-turn.ts` のwatchdogで応答不能時も危険札を残しにくい合法手へ進み、通常完了以外は`ai_fallback`へ記録する。ルームAIの探索予算は`softMs: 50`、`hardMs: 600`、候補評価上限`64`、初手を含む打ち切り`65`手。同じ想定worldで全合法手を評価し終えた周だけを採用し、通常の予算完了ではworkerを再利用する。hard期限までに予定worldを完了できなければ、公平に完了した周だけを`partial-search`として返し、応答自体がなければ`heuristic`へ退避する。
